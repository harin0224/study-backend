...
-- 사원명이 shith인 사람의, 같은 부서의 직원 조희하고 싶음
SELECT *
FROM EMP
WHERE ENAME = 'SHITT';
...
-- join 복습
SELECT A.EMPNO 사번, A.ENAME 이름, A.ENAME 부서, A.SAL 급여
FROM EMP A
LEFT JOIN DEPT B ON A.DEPTNO = B.DEPTNO 
WHERE A.SAL > (SELECT SAL FROM EMP WHERE ENAME = 'SMITH');
...
-- 부서별 급여의 합계가 가장 큰 부서의 부서명, 급여 합계를 구하세요 (Having에 서브 쿼리 걸기)
SELECT B.DEPTNO, B.DNAME, SUM(A.SAL) TOTSAL
FROM EMP A
INNER JOIN DEPT B ON A.DEPTNO = B.DEPTNO 
GROUP BY B.DEPTNO , B.DNAME 
HAVING SUM(A.SAL) = 
(SELECT MAX(SUM(SAL)) FROM EMP GROUP BY B.DEPTNO);
...
-- 부서별 최고 급여를 받는 직원의 정보를 조회해라
-- 서브 쿼리의 행이 여러행이기 때문 (부서별로)
SELECT * 
FROM EMP
...
-- MGR(관리자)가 0 아니어야 하고 null 아니면 됨
-- 중복 없이
...
CASE WHEN A.EMPNO IN 
(SELECT DISTINCT MGR FROM EMP WHERE MGR IS NOT NULL AND MGR != 0 ) THEN '관리자'
		ELSE '직원'
	END 구분	
FROM EMP A
LEFT JOIN DEPT B ON A.DEPTNO = B.DEPTNO 
...
SELECT * FROM EMP A WHERE SAL > (SELECT FROM EMP B WHERE B.JOB = A.JOB)
...
SELECT ROWNUM, A.DEPTNO, AVG(SAL)
FROM (
	SELECT A.DEPTNO, AVG(SAL)
