...
--GRANT CREATE VIEW TO HR;	시스템 계정 가서 권한주기

CREATE OR REPLACE VIEW V_EMP
(사번, 이름, 부서)
AS 
SELECT employee_id, FIRST_name || ' ' || last_name name, Departmemt_id 
FROM EMPLOYEES;
...
LEFT JOIN DEPARTMENTS C ON A.DEPARTMEMT_ID = C.DEPARTMENT_ID
...
--> 서브쿼리에 기술한 조건에 부합하지않기때문에 에러
UPDATE VW_EMP2
SET SALARY =1000
WHERE EMPLOYEE_ID =101;

--> 서브쿼리에 기술한 조건에 만족하므로 변경됨
UPDATE VW_EMP2
SET SALARY =100000
