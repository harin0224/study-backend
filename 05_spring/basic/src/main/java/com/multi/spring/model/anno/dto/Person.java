...
public class Person {
	...
	@Value("선재") //@Value("값")을 넣어서 내부에서 xml 화 하여 값을 가지고 시작하게 만드는 어노테이션  (xml에서 set 했던것과 같음) 단순 값 주입
	private String name;
...
//  @Resource : 필드, 입력 파라미터가 한 개인 빈 프로퍼티 setter 메소드에 적용가능
	...
	@Autowired	// (required = false) 자동 주입 대상이 필수가 아닐 때 작성
	@Qualifier("pizza")	// 자동주입할 하나의 인터페이스 타입의 빈이 2개 이상인 경우 하나의 빈을 지정하는 어노테이션
	private Food myFood;
